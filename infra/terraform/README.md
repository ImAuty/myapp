...
以下の`aws_instance.app`のID等は、インスタンスが置き換わる度（AMI更新、`encrypted`変更など`must be replaced`を伴う変更の度）に変わります。実行前に`aws ec2 describe-instances --filters "Name=tag:Name,Values=myapp-ec2" --query 'Reservations[].Instances[].InstanceId'`等で現在のIDを確認してください。
...
terraform import aws_instance.app i-021b06797abd2b1ff
